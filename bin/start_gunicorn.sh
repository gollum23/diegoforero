...
DJANGODIR=$HOME/diegoforero.xyz             # Django project directory
SOCKFILE=$HOME/diegoforero.xyz/run/gunicorn.sock  # we will communicte using this unix socket
...
exec $WORKON_HOME/diegoforero/bin/gunicorn ${DJANGO_WSGI_MODULE}:application \
  --name $NAME \
  --workers $NUM_WORKERS \
